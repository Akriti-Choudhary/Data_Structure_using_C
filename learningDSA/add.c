...
//displaying in the triplet format
void displaySparse(int arr[][3], int d, int ele)
{
    for (int i = 0; i <= ele; ++i)
    {
...
//for adding the matrix and it's transpose
void add(int arrTra[][3], int arr[][3], int d, int ele)
{
    int sum[ele + ele][3];
    ...
    while (i <= t1 && j <= t2)
    {
        if (arr[i][0] < arrTra[j][0]) //row numbers are not equal
        {
            sum[k][0] = arr[i][0];
            sum[k][1] = arr[i][1];
            sum[k][2] = arr[i][2];
        ...
        else if (arrTra[j][0] < arr[i][0]) //row numbers are not equal
        {
            sum[k][0] = arrTra[j][0];
            sum[k][1] = arrTra[j][1];
            sum[k][2] = arrTra[j][2];
        ...
        else if (arr[i][1] < arrTra[j][1]) //row numbers are equal, compare column
        {
            sum[k][0] = arr[i][0];
            sum[k][1] = arr[i][1];
            sum[k][2] = arr[i][2];
        ...
        else if (arrTra[j][1] < arr[i][1]) //row numbers are equal, compare column
        {
            sum[k][0] = arrTra[j][0];
            sum[k][1] = arrTra[j][1];
            sum[k][2] = arrTra[j][2];
        ...
        {
            sum[k][0] = arr[i][0]; //row and column numbers are equal
            sum[k][1] = arr[i][1];
    ...
    while (i <= t1) //copy remaining terms from arr
    {
        sum[k][0] = arr[i][0];
        sum[k][1] = arr[i][1];
        sum[k][2] = arr[i][2];
    ...
    while (j <= t2) //copy remaining terms from arrTra
    {
        sum[k][0] = arrTra[j][0];
        sum[k][1] = arrTra[j][1];
        sum[k][2] = arr[j][2];
    ...
    sum[0][2] = k - 1; //set number of terms in sum
    puts("Sum of the matrix and it's transpose : ");
